...
window.onload = function () {
	...
	// reviews-popup
	var reviewsBtn = document.querySelector('.reviews__add-btn');
	var reviewsPopup = document.querySelector('.reviews__popup');
	var reviewsPopupBtn = document.querySelector('.reviews__popup-btn');
	var reviewsFile = document.querySelector('.file');
	reviewsBtn.addEventListener('click', function (e) {
		e.preventDefault();
		var reviewsInput = document.querySelector('.reviews__add-text').value;
		if (reviewsInput.length >= 1 && reviewsInput !== ' ') {
			reviewsPopup.classList.add('show');
		} else {
			alert('Schreiben Sie eine Rezension.');
		}
	});
	reviewsPopupBtn.addEventListener('click', function () {
		reviewsPopup.classList.remove('show');
		var reviewsInput = document.querySelector('.reviews__add-text');
		reviewsInput.value = ' ';
	});

	reviewsFile.addEventListener('change', function () {
		var splittedFakePath = this.value.split('\\');
		alert('Ihre Datei "' + splittedFakePath[splittedFakePath.length - 1] + '" wurde erfolgreich hinzugefügt.');
	});
};
//components popups
var compAuthBtn = document.querySelector('.components__authors-btn'),
    comAuthPop = document.querySelector('.components__authors'),
    compAuthLink = document.querySelector('.author'),
    compResBtn = document.querySelector('.components__results-btn'),
    comResPop = document.querySelector('.components__results'),
    compResLink = document.querySelector('.results');

compAuthLink.addEventListener('click', function () {
	comAuthPop.classList.add('show');
});
compAuthBtn.addEventListener('click', function () {
	comAuthPop.classList.remove('show');
});

compResLink.addEventListener('click', function () {
	comResPop.classList.add('show');
});
compResBtn.addEventListener('click', function () {
	comResPop.classList.remove('show');
});
...
var d = new Date(),
    year5 = document.querySelector('.year-5');
...
let mainDocLang = 'de'; // СЏР·С‹Рє РІС‹РІРѕРґР° С‚РµРєСЃС‚Р° РІ Р±Р°РЅРЅРµСЂРµ
